...
export default function AddMABArms({ onValidate }: StepComponentProps) {
  ...
  const bayesABarms: Record<number, string> = {1: "Treatment Arm", 2: "Control Arm"};
    ...
    experimentState.arms.forEach((arm, index) => {
      ...
      if (experimentState.prior_type === "beta") {
        if ("alpha_init" in arm) {
          ...
          if (arm.alpha_init && arm.alpha_init <= 0) {
        ...
        if ("beta_init" in arm) {
          ...
          if (arm.beta_init && arm.beta_init <= 0) {
        ...
        if ("sigma_init" in arm) {
          ...
          if (arm.sigma_init && arm.sigma_init <= 0) {
  ...
  return (
    <div>
      <div className="flex w-full flex-wrap items-end justify-between gap-4 border-b border-zinc-950/10 pb-6 dark:border-white/10">
        <h2 className="text-2xl font-semibold tracking-tight">Add Experiment Arms</h2>
        <div className="flex gap-4">
          <Button className="mt-4" onClick={addArm} disabled={isBayesianABState(experimentState)}>
            <Plus className="w-4 h-4 mr-2" />
            Add Arm
          </Button>
          <Button
            className="mt-4 mx-4"
            disabled={experimentState.arms.length <= 2 || isBayesianABState(experimentState)}
            variant="outline"
            onClick={() => {
      ...
      <div className="space-y-6" aria-label="Add Experiment Arms">
        {experimentState.arms.map((arm, index) => (
          <div key={index}>
            <DividerWithTitle title={isBayesianABState(experimentState) ? bayesABarms[index + 1] : `Arm ${index + 1}`} />
                    ...
                    <div className="flex flex-row items-start">
                      ...
                      <div className="basis-3/4 flex flex-col">
                        <Input
                          ...
                          value={(arm as NewArm).alpha_init || ""}
                          onChange={(e) => {
                            updateArm(index, {
                      ...
                      <div className="basis-3/4 flex flex-col">
                        <Input
                          ...
                          value={
                            ...
                            (arm as NewArm).mu_init?.toString()
                          }
                          onChange={(e) => {
                    ...
                    <div className="flex flex-row items-start">
                      ...
                      <div className="basis-3/4 flex flex-col">
                        <Input
                          ...
                          value={(arm as NewArm).sigma_init || ""}
                          onChange={(e) => {
                            updateArm(index, {
