import { ExperimentState, Arm } from "../types";
import {
  Card,
  CardContent,
  CardHeader,
...
import { isMABExperimentStateBeta } from "../store/useExperimentStore";
...
export function BaseExperimentCard<T extends { name: string }>({
  experiment,
...
export function ExperimentCard({ experiment }: { experiment: ExperimentState }) {
  const maxValue = !isMABExperimentStateBeta(experiment)
    ? Math.max(
        ...experiment.arms.map((arm: Arm) =>
          arm.mu ? arm.mu.reduce((a, b) => a + b, 0) : 0
        ),
        0
      )
    : 0;

  return isMABExperimentStateBeta(experiment) ? (
    <BaseExperimentCard
      experiment={experiment}
      calculateProgressValue={(arm: Arm) =>
        arm.alpha && arm.beta ? (arm.alpha * 100) / (arm.alpha + arm.beta) : 0
      }
      formatDisplayValue={(arm: Arm) =>
        arm.alpha && arm.beta
    ...
    <BaseExperimentCard
      ...
      calculateProgressValue={(arm: Arm) =>
        maxValue && arm.mu
          ? arm.mu.reduce((a, b) => (a + b) * 100, 0) / (maxValue * 1.5)
          : 0
      }
      formatDisplayValue={(arm: Arm) =>
        arm.mu ? `${arm.mu.reduce((a, b) => a + b, 0).toFixed(1)}` : "N/A"
      }
      maxValue={maxValue}
    />
  );
